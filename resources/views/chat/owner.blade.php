...
<h4 class="mb-3">
    Chat dengan {{ $chat->user->name }} – {{ $chat->property->name }}
</h4>
...
<div class="card">
        ...
        @forelse($chat->messages as $msg)
            <div class="mb-2 {{ $msg->sender_id == auth()->id() ? 'text-end' : '' }}">
                <div class="d-inline-block p-2 rounded
                    {{ $msg->sender_id == auth()->id() ? 'bg-success text-white' : 'bg-light' }}">
                    {{ $msg->message }}
                </div>
    ...
    <div class="card-footer">
        <form method="POST" action="{{ route('owner.chat.send', $chat->id) }}">
            @csrf
